...
//  Loop
//
//  Manages trim-aware playback: play from start to stop, then advance to next
//
...
final class PlaybackController: ObservableObject {
    ...
    func playFromIndex(_ index: Int) {
        ...
        // Seek to start after a short delay (play needs to start first)
    ...
    func playPrevious() {
        ...
        if index > 0 {
            ...
            // Restart current clip from start
            playFromIndex(0)
        }
    }
    ...
    func checkPositionAndAdvance() {
        ...
        // Check if we've reached the stop point (with buffer)
